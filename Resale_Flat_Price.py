...
def predict_price(year, town, flat_type, flr_area_sqm, flat_model, stry_start, stry_end, re_les_year, re_les_month, les_coms_dt):
    try:
        ...
        town_2 = town_mapping(town)          # assumes you already have town_mapping()
        flt_ty_2 = flat_type_mapping(flat_type)  # assumes you already have flat_type_mapping()
        flr_ar_sqm_1 = float(flr_area_sqm)

        flt_model_2 = flat_model_mapping(flat_model)  # assumes you already have flat_model_mapping()

        # Use log safely (avoid log(0) or negatives)
        str_str = np.log(max(1, int(stry_start)))
        str_end = np.log(max(1, int(stry_end)))

        # Default values for missing lease details
        rem_les_year = 80
        rem_les_month = 0
        lese_coms_dt = 2000
        ...
        # --- Prepare user data ---
        user_data = np.array([[
            year_1, town_2, flt_ty_2, flr_ar_sqm_1,
            flt_model_2, str_str, str_end,
            rem_les_year, rem_les_month, lese_coms_dt
        ]], dtype=np.float32)

        # Replace infinities / NaNs with 0 (safety net)
        user_data = np.nan_to_num(user_data, nan=0.0, posinf=0.0, neginf=0.0)
